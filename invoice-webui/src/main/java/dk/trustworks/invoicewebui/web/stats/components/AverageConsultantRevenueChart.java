    ...
    public Chart createRevenuePerConsultantChart() {
        ...
        do {
            for (User user : userService.findCurrentlyEmployedUsers(ConsultantType.CONSULTANT)) {
                ...
                LocalDate periodEnd = currentDate.plusYears(1);
                ...
                if(averagePerYear.isPresent()) {
                    System.out.println(user.getUsername() + " ("+currentDate.getYear()+") = " + averagePerYear.getAsDouble());
                    ...
                    averagePerYearMap.put(currentDate, averagePerYearMap.getOrDefault(currentDate, 0.0) + averagePerYear.getAsDouble());
                }
            }
        ...
        for (User user : averagePerUserPerYear.keySet().stream().sorted(Comparator.comparing(User::getUsername)).collect(Collectors.toList())) {
            ...
            String[] categories = userAverageByYearMap.keySet().stream().sorted().map(localDate -> Integer.toString(localDate.getYear())).toArray(String[]::new);
            Number[] values = new Number[userAverageByYearMap.size()];
            int i = 0;
            for (LocalDate localDate : userAverageByYearMap.keySet().stream().sorted().collect(Collectors.toList())) {
