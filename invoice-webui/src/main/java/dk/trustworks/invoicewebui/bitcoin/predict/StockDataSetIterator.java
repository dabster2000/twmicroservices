package dk.trustworks.invoicewebui.bitcoin.predict;

import com.google.common.collect.ImmutableMap;
import dk.trustworks.invoicewebui.bitcoin.representation.PriceCategory;
import dk.trustworks.invoicewebui.model.rest.cryptocompare.CryptoCompareRoot;
import dk.trustworks.invoicewebui.model.rest.cryptocompare.Datum;
...
public class StockDataSetIterator implements DataSetIterator {
    ...
    public StockDataSetIterator (CryptoCompareRoot filename, String symbol, int miniBatchSize, int exampleLength, double splitRatio, PriceCategory category) {
    ...
    private List<StockData> readStockDataFromFile (CryptoCompareRoot filename, String symbol) {
        List<StockData> stockDataList = new ArrayList<>();

        for (Datum datum : filename.getData()) {
            StockData stockData = new StockData(
                    new DateTime(datum.getTime() * 1000L).toString("yyyy-MM-dd"),
                    "BTC",
                    datum.getOpen(),
                    datum.getClose(),
                    datum.getLow(),
