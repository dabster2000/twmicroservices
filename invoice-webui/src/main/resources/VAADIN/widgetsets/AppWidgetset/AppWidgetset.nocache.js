function AppWidgetset(){
  ...
  if ($wnd) {
    var devModePermitted = !!($wnd.location.protocol == $intern_65 || $wnd.location.protocol == $intern_66);
    ...
    if (devModePermitted && supportsSessionStorage()) {
      var devModeKey = $intern_68;
      var devModeUrl = $wnd.sessionStorage[devModeKey];
      if (!/^http:\/\/(localhost|127\.0\.0\.1)(:\d+)?\/.*$/.test(devModeUrl)) {
        if (devModeUrl && (window.console && console.log)) {
          console.log($intern_69 + devModeUrl);
        }
        devModeUrl = $intern_15;
      }
      if (devModeUrl && !$wnd[devModeKey]) {
        $wnd[devModeKey] = true;
        $wnd[devModeKey + $intern_70] = computeScriptBase();
